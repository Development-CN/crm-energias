...
from .models import Informacion, Items, ManoDeObra, Refacciones
...
STYLEHD = STYLES["Mazda"]
STYLEHD.alignment = TA_LEFT
STYLEBH = STYLES["Mazda"]
...
def cotizacion_pdf(no_orden, refacciones_ids, mano_de_obra_ids):
    ...
    items_tecnico = Items.objects.filter(no_orden=no_orden).exclude(estado="Buen Estado")
        ...
        if refaccion.item == item:
            refacciones_rows.append(
                [
                    ...
                    Paragraph(str(localize(intcomma(refaccion.precio_unitario))), STYLEBH),
                    Paragraph(str(refaccion.porcentaje_descuento), STYLEBH),
                    Paragraph(str(localize(intcomma(refaccion.subtotal))), STYLEBH),
                ]
            )
        ...
        if mo.item == item:
            mano_de_obra_rows.append(
                [
                    ...
                    Paragraph(str(localize(intcomma(round(mo.precio_ut * mo.cantidad_uts, 2)))), STYLEBH),
                    Paragraph(str(mo.porcentaje_descuento), STYLEBH),
                    Paragraph(str(localize(intcomma(mo.subtotal))), STYLEBH),
                ]
            )
...
def get_table_totales(refacciones, mano_de_obra):
    ...
    if refacciones:
        ...
        subtotales_ref = [
            [
                "",
                "",
                Paragraph("SUBTOTAL REPUESTOS", STYLEHD),
                Paragraph(localize(intcomma(subtotal_ref_no_desc)), STYLEHD),
            ],
            [
                "",
                "",
                Paragraph("DESCUENTO REPUESTOS", STYLEHD),
                Paragraph(localize(intcomma(subtotal_ref_no_desc - subtotal_ref_desc)), STYLEHD),
            ],
        ]
    ...
    if mano_de_obra:
        ...
        subtotales_mo = [
            [
                "",
                "",
                Paragraph("SUBTOTAL MANO DE OBRA", STYLEHD),
                Paragraph(localize(intcomma(round(subtotal_mo_no_desc, 2))), STYLEHD),
            ],
            [
                "",
                "",
                Paragraph("DESCUENTO MANO DE OBRA", STYLEHD),
                Paragraph(localize(intcomma(round(subtotal_mo_no_desc - subtotal_mo_desc, 2))), STYLEHD),
            ],
        ]
    ...
    totales_fin = [
        [
            "",
            "",
            Paragraph("SUBTOTAL NETO", STYLEHD),
            Paragraph(localize(intcomma(round(subtotal_ref_no_desc + subtotal_mo_no_desc, 2))), STYLEHD),
        ],
        [
            Paragraph("*** VALIDEZ DE PROFORMA 15 DIAS", STYLEHD),
            "",
            Paragraph("IVA", STYLEHD),
            Paragraph(str(porcentaje_iva)[2:] + "%", STYLEHD),
        ],
        [
            Paragraph("*** DESCUENTO APLICA SOLO PAGOS EN EFECTIVO, TRASFERENCIA O CHEQUE", STYLEHD),
            "",
            Paragraph("TOTAL A PAGAR:", STYLEHD),
            Paragraph(localize(intcomma(subtotal_mo_iva + subtotal_ref_iva)) + " " + settings.CURRENCY, STYLEHD),
        ],
    ]
