...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("seguimientolite_mazda_col", "0001_initial"),
    ]
    ...
    operations = [
        migrations.RunSQL(
            ...
            modelo= '',
            color = '',
            tecnico = (select top 1 idtecnico from [{TABLERO_DB}].[dbo].[Tb_CITAS] where noorden = a.NOORDEN and servicioCapturado <>'lavado'),
        ...
        migrations.RunSQL(
            ...
            where no_orden in (select no_orden from actividades_cotizaciones where fecha_hora_fin is not null) or  no_orden in (select no_orden from v_revisiones_limpias)
            """
        ),
        migrations.RunSQL(
            f"""
            create view [dbo].[v_revisiones_limpias] as 
            with cte_estados as (
                -- Calcula califica el estado de cada item
                select 
                no_orden,
                verdes = case when estado = 'Buen Estado' then 1 else 0 end,
                amarillos = case when estado = 'Recomendado' then 1 else 0 end,
                rojos = case when estado = 'Inmediato' then 1 else 0 end,
                no_aplica = case when estado ='Corregido' then 1 else 0 end
                from actividades_tecnico
            ),

            cte_suma_estados as (
                -- Suma los estados y los agrupa por orden
                select 
                    no_orden,
                    verdes	=sum(a.verdes),
                    amarillos=sum(a.amarillos),
                    rojos=sum(a.rojos),
                    no_aplica=sum(a.no_aplica)

                from cte_estados as a 
                group by no_orden
            )

            --selecciona solo las ordenes que tienen revisiones "limpias" es decir, todo en verde
            select * from cte_suma_estados 
            where amarillos = 0 and rojos = 0
            """
        ),
    ]
