...
class CatalogoModelo(models.Model):
    ...
    # Almacenar múltiples opciones en un campo de texto (separadas por comas)
    years = models.CharField(max_length=255)

    def set_years(self, year_list):
        # Asegurarse de que todas las opciones en year_list son válidas
        valid_choices = [choice[0] for choice in self.years_choices]
        if all(year in valid_choices for year in year_list):
            self.years = ','.join(year_list)
        else:
...
class VehiculosInteresLead(models.Model):
    ...
    cilindros = models.CharField(max_length=100, null=True)
    tipo_inyeccion = models.CharField(max_length=100, null=True)
    kit_conversion = models.CharField(max_length=100, null=True)
...
class Evento(models.Model):
    ...
    lead = models.ForeignKey("Lead", on_delete=models.CASCADE)
    choices_tipo = (
        ("Llamada cliente", "Llamada cliente"),
        ("Mensaje cliente", "Mensaje cliente"),
        ("Visita cliente", "Visita cliente"),
        ("Recepción del cliente", "Recepción del cliente"),
        ("Envio de propuesta economica", "Envio de propuesta economica"),
        ("Contacto proximo servicio", "Contacto proximo servicio")
    )
